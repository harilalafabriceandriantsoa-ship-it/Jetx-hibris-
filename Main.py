...
<style>
    ...
    .metric-box { background: rgba(255, 255, 255, 0.05); padding: 10px; border-radius: 10px; border: 1px solid #00ffcc33; }
</style>
...
if not st.session_state.authenticated:
    st.markdown("<h1 style='text-align:center; color:#ff0066;'>X3+ LASER LOGIN</h1>", unsafe_allow_html=True)
...
def run_x3_laser_engine(h_in, t_in, last_cote):
    ...
    # Advanced simulation based on hash complexity
    sims = np.random.lognormal(np.log(2.15), 0.22, 120000)
    prob_x3 = round(float(np.mean(sims >= 3.0)) * 100, 2)
    
    # Adaptive Confidence Strength
    strength = round(max(30, min(99.5, prob_x3 * 1.25 + (5 if last_cote < 2.0 else -5))), 2)
    
    try:
        bt = datetime.strptime(t_in.strip(), "%H:%M:%S")
        # Entry time calculated with micro-shift logic
...
with st.sidebar:
    ...
    if st.button("🗑️ RESET ALL DATA", use_container_width=True):
        ...
        st.rerun()
    st.markdown("---")
    st.info("V16.1 Laser Focus Active")
...
with c1:
    ...
    h_input = st.text_input("SERVER HASH", value="", placeholder="Paste game hash...")
    t_input = st.text_input("TIME (HH:MM:SS)", value="", placeholder="Current time...")
    lc_input = st.number_input("LAST COTE", value=0.0, step=0.01)
    
    if st.button("🚀 ANALYSE", use_container_width=True):
        if h_input and t_input:
            st.session_state.last = run_x3_laser_engine(h_input, t_input, lc_input)
            st.rerun()
    
    if st.session_state.last:
        st.markdown("---")
        st.markdown("### 💾 FEEDBACK")
        rc_input = st.number_input("REAL COTE (Vokatra nipoaka)", value=0.0, step=0.01)
        if st.button("SAVE RESULT", use_container_width=True):
            ...
            # Simple ML Training update if enough data
            ...
            save_history(st.session_state.history)
            st.success("Data stored for AI training!")
...
with c2:
    if st.session_state.last:
        ...
        m1.markdown(f"<div class='metric-box'><small>SAFE</small><br><b>{r['safe']}x</b></div>", unsafe_allow_html=True)
        m2.markdown(f"<div class='metric-box'><small>TARGET</small><br><b style='color:#00ffcc;'>{r['x3_target']}x</b></div>", unsafe_allow_html=True)
        m3.markdown(f"<div class='metric-box'><small>MAX</small><br><b>{r['max']}x</b></div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
