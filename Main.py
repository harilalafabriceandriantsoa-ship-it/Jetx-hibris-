...
# STYLE CSS - NOHAFANTARINA NY LOKO MAINTY AMIN'NY INPUT
st.markdown("""
<style>
...
/* FANITSIANA INPUT: Soratra mainty sady hita tsara */
...
def engine(h_in, last_time, lc):
    ...
    acc_min, acc_moy, acc_max = 70.0, 50.0, round(p3 * 0.85, 1)
    ...
    return {"lc": lc, "ent": ent, "sh": sh, "sig": sig, "sc": sc,
            "bp": bp, "p35": p35, "p4": p4, "str": str_, "cur": cur, "hp": hp,
            "tmin": tmin, "tmoy": tmoy, "tmax": tmax, "acc_min": acc_min,
            "acc_moy": acc_moy, "acc_max": acc_max, "res": None, "hi": len(st.session_state.H)}
...
with ci:
    st.markdown("<div class='card'>", unsafe_allow_html=True)
    h_in  = st.text_input("🔐 SERVER HASH", placeholder="Ampidiro eto ny Hash...")
    lt_in = st.text_input("⏰ LAST TIME (HH:MM:SS)", placeholder="Ohatra: 18:30:15")
...
with co:
    ...
    if r:
        ...
        with c3: st.markdown(f"<div class='tbox'><div class='tl'>MAX</div><div class='tv' style='color:#ff00ff;'>{r['tmax']}×</div></div>", unsafe_allow_html=True)
        ...
        st.markdown("<div class='card' style='min-height:300px;text-align:center;'><br><br>🚀 EN ATTENTE...</div>", unsafe_allow_html=True)
